...
public class BlackClover extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CustomArrayAdapter adapter = new CustomArrayAdapter(BlackClover.this, android.R.layout.simple_expandable_list_item_1, list);
        listView.setAdapter(adapter);
        if (intent != null) {
            if (intent.hasExtra("chapter")) {
                int chapterNumber = intent.getIntExtra("chapter", -1);

                if (chapterNumber != -1) {
                    adapter.setReadingChapterIndex(chapterNumber);
                }
            }
        }
